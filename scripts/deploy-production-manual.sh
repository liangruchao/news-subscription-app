...
if [ "$GITHUB_ACCESSIBLE" = true ]; then
    ...
    if [ -d ".git" ] && git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        git clone -b main git@github.com:liangruchao/news-subscription-app.git tmp-repo
...
if [ -f "deploy/production/.env" ]; then
    ...
    log_error "deploy/production/.env 不存在"
    exit 1
fi

if [ -f "deploy/production/docker-compose.yml" ]; then
    cp deploy/production/docker-compose.yml docker-compose.yml
    log_success "docker-compose.yml 已配置"
else
    log_error "deploy/production/docker-compose.yml 不存在"
    exit 1
fi

# 复制 nginx 配置
mkdir -p nginx/conf.d
if [ -f "deploy/production/nginx/conf.d/default.conf" ]; then
    cp deploy/production/nginx/conf.d/default.conf nginx/conf.d/default.conf
...
# 给镜像打标签
docker tag news-subscription-app-backend:latest news-app:production-manual
...
log_success "Production 环境部署完成！"
echo ""
echo "服务地址:"
echo "  Web:    http://139.224.189.183"
echo "  API:    http://139.224.189.183:8081"
echo ""
echo "管理命令:"
